...
pub fn compute_unit_limit_instruction(units :: Int) -> Instruction!String do
  if units < 0 || units > 4_294_967_295 do
    ...
    Ok(Instruction {
      ...
      data: (((units
        |> Int.to_string()
        |> Bytes.write_uint_le(4))?) |2> append_bytes(uint8(2)?))?
    })
  end
end
...
pub fn compute_unit_price_instruction(micro_lamports :: U64) -> Instruction!String do
  Ok(Instruction {
    ...
    data: (((micro_lamports
      |> U64.to_string()
      |> Bytes.write_uint_le(8))?) |2> append_bytes(uint8(3)?))?
  })
end

fn checked_account_meta(key :: Pubkey, signer :: Bool, writable :: Bool) -> AccountMeta!String do
...
pub fn transfer_checked_instruction(source :: Pubkey,
  mint :: Pubkey,
  destination :: Pubkey,
  authority :: Pubkey,
  amount :: U64,
  decimals :: Int) -> Instruction!String do
  if decimals < 0 || decimals > 255 do
    ...
    let with_discriminator = (amount_bytes |2> append_bytes(uint8(12)?))?
    Ok(Instruction {
...
pub fn create_associated_token_idempotent_instruction(payer :: Pubkey,
  associated_account :: Pubkey,
  wallet :: Pubkey,
  mint :: Pubkey) -> Instruction!String do
...
fn merge_key_meta(values :: List<KeyMeta>,
  key :: Pubkey,
  signer :: Bool,
  writable :: Bool,
  invoked :: Bool,
  index :: Int,
  found :: Bool,
  output :: List<KeyMeta>) -> List<KeyMeta> do
...
fn collect_instruction_metas(values :: List<KeyMeta>,
  ...
  if index >= List.length(instructions) do
    ...
    values
      ...
      |> merge_key_meta(instruction.program_id, false, false, true, 0, false, List.new())
...
fn append_pubkey_values(output :: List<Pubkey>, values :: List<Pubkey>, index :: Int) -> List<Pubkey> do
  if index >= List.length(values) do
    ...
    values
      |> List.get(index) |2> List.append(output)
...
fn ordered_keys(values :: List<KeyMeta>) -> List<Pubkey> do
  values
    ...
    |> append_pubkey_values(category_keys(values, 1, 0, List.new()), 0)
    |> append_pubkey_values(category_keys(values, 2, 0, List.new()), 0)
    |> append_pubkey_values(category_keys(values, 3, 0, List.new()), 0)
end

fn category_count(values :: List<KeyMeta>, category :: Int, index :: Int, count :: Int) -> Int do
  if index >= List.length(values) do
    count
  else
...
fn header_from_metas(values :: List<KeyMeta>) -> MessageHeader do
  MessageHeader {
    num_required_signatures: category_count(values, 0, 0, 0) + category_count(values, 1, 0, 0),
...
fn compile_account_indexes(accounts :: List<AccountMeta>,
  keys :: List<Pubkey>,
  index :: Int,
  output :: List<Int>) -> List<Int>!String do
  if index >= List.length(accounts) do
    Ok(output)
  else
    compile_account_indexes(accounts,
      keys,
      index + 1,
      output
        |> List.append(pubkey_index(keys, List.get(accounts, index).pubkey, 0)?))
  end
end

fn compile_instructions(instructions :: List<Instruction>,
  keys :: List<Pubkey>,
  index :: Int,
  output :: List<CompiledInstruction>) -> List<CompiledInstruction>!String do
  if index >= List.length(instructions) do
    Ok(output)
  else
    let instruction = instructions
    ...
    compile_instructions(instructions,
      ...
      output
        |> List.append(CompiledInstruction {
          program_id_index: pubkey_index(keys, instruction.program_id, 0)?,
          account_indexes: compile_account_indexes(instruction.accounts, keys, 0, List.new())?,
...
pub fn compile_legacy_message(payer :: Pubkey,
  recent_blockhash :: Hash,
  instructions :: List<Instruction>) -> LegacyMessage!String do
  if List.length(instructions) == 0 do
    ...
    Ok(LegacyMessage {
      ...
      instructions: compile_instructions(instructions, keys, 0, List.new())?
    })
  end
end
...
fn append_pubkeys(output :: Bytes, keys :: List<Pubkey>, index :: Int) -> Bytes!String do
  if index >= List.length(keys) do
    ...
    if Bytes.length(key.bytes) != 32 do
      ...
      append_pubkeys(append_bytes(output, key.bytes)?, keys, index + 1)
    end
  end
end

fn validate_pubkeys(keys :: List<Pubkey>, index :: Int, label :: String) -> Int!String do
  if index >= List.length(keys) do
    Ok(index)
  else
    let key = keys
...
fn append_account_indexes(output :: Bytes, indexes :: List<Int>, index :: Int, account_count :: Int) -> Bytes!String do
  if index >= List.length(indexes) do
    ...
    if account_index < 0 || account_index >= account_count do
      ...
      append_account_indexes(append_uint8(output, account_index)?,
        indexes,
        index + 1,
        account_count)
    end
...
fn append_compiled_instructions(output :: Bytes,
  instructions :: List<CompiledInstruction>,
  index :: Int,
  program_count :: Int,
...
pub fn serialize_legacy_message(message :: LegacyMessage) -> Bytes!String do
  ...
  let with_signatures = append_uint8(Bytes.empty(), message.header.num_required_signatures)?
  let with_readonly_signers = append_uint8(with_signatures,
    message.header.num_readonly_signed_accounts)?
  let with_readonly_unsigned = append_uint8(with_readonly_signers,
    message.header.num_readonly_unsigned_accounts)?
  let with_account_count = append_short_u16(with_readonly_unsigned, account_count)?
  let with_account_keys = append_pubkeys(with_account_count, message.account_keys, 0)?
  let with_blockhash = append_bytes(with_account_keys, message.recent_blockhash.bytes)?
  let with_instruction_count = append_short_u16(with_blockhash, List.length(message.instructions))?
  let bytes = append_compiled_instructions(with_instruction_count,
  ...
  if index >= List.length(lookups) do
    ...
    if Bytes.length(lookup.account_key.bytes) != 32 do
      ...
      if writable_count > 256 || readonly_count > 256 do
        ...
        loaded_account_count(lookups, index + 1, total + writable_count + readonly_count)
      end
    end
  end
end

fn pubkey_occurrences(values :: List<Pubkey>, key :: Pubkey, index :: Int, count :: Int) -> Int do
  if index >= List.length(values) do
    count
  else
...
fn lookup_occurrences(lookups :: List<AddressTableLookup>,
  key :: Pubkey,
  index :: Int,
  count :: Int) -> Int do
  if index >= List.length(lookups) do
...
fn validate_loaded_addresses(metas :: List<KeyMeta>,
  ...
  if index >= List.length(addresses) do
    ...
    if lookup_occurrences(lookups, key, 0, 0) != 1 do
      ...
      let meta_index = key |2> key_meta_index(metas, 0)
      if meta_index < 0 do
        validate_loaded_addresses(metas, lookups, addresses, writable, index + 1)
      else
        let meta = metas
          |> List.get(meta_index)
...
fn validate_loaded_metas(metas :: List<KeyMeta>, lookups :: List<AddressTableLookup>, index :: Int) -> Int!String do
  if index >= List.length(lookups) do
    ...
    let lookup = lookups
      |> List.get(index)
    validate_loaded_addresses(metas, lookups, lookup.writable_addresses, true, 0)?
    validate_loaded_addresses(metas, lookups, lookup.readonly_addresses, false, 0)?
    validate_loaded_metas(metas, lookups, index + 1)
  end
end

fn static_key_metas(metas :: List<KeyMeta>,
  lookups :: List<AddressTableLookup>,
  index :: Int,
  output :: List<KeyMeta>) -> List<KeyMeta> do
...
fn loaded_pubkeys(lookups :: List<AddressTableLookup>,
  writable :: Bool,
  index :: Int,
  ...
  if index >= List.length(lookups) do
    ...
    let lookup = lookups
      |> List.get(index)
    output
      |> append_pubkey_values(if writable do
          lookup.writable_addresses
        else
          lookup.readonly_addresses
        end,
        0) |4> loaded_pubkeys(lookups, writable, index + 1)
  end
end

pub fn compile_message_v0(payer :: Pubkey,
  recent_blockhash :: Hash,
  instructions :: List<Instruction>,
  address_table_lookups :: List<AddressTableLookup>) -> MessageV0!String do
  if List.length(instructions) == 0 do
    Err("SOLANA_TX: message requires at least one instruction")
  else
    let metas = payer
      |> compiled_key_metas(instructions)
    let loaded_count = loaded_account_count(address_table_lookups, 0, 0)?
    validate_loaded_metas(metas, address_table_lookups, 0)?
    let static_metas = static_key_metas(metas, address_table_lookups, 0, List.new())
    let static_keys = static_metas
      |> ordered_keys()
    let keys = static_keys
      |> append_pubkey_values(loaded_pubkeys(address_table_lookups, true, 0, List.new()), 0)
      |> append_pubkey_values(loaded_pubkeys(address_table_lookups, false, 0, List.new()), 0)
...
fn append_lookup_indexes(output :: Bytes, indexes :: List<Int>, index :: Int) -> Bytes!String do
  if index >= List.length(indexes) do
    Ok(output)
  else
    ...
    if lookup_index < 0 || lookup_index > 255 do
      ...
      append_lookup_indexes(append_uint8(output, lookup_index)?, indexes, index + 1)
    end
  end
end
...
fn append_address_table_lookups(output :: Bytes, lookups :: List<AddressTableLookup>, index :: Int) -> Bytes!String do
  if index >= List.length(lookups) do
    ...
    let with_writable_count = append_short_u16(with_key, List.length(lookup.writable_indexes))?
    let with_writable_indexes = append_lookup_indexes(with_writable_count,
      lookup.writable_indexes,
      0)?
    let with_readonly_count = append_short_u16(with_writable_indexes,
      List.length(lookup.readonly_indexes))?
    append_address_table_lookups(append_lookup_indexes(with_readonly_count,
...
pub fn serialize_message_v0(message :: MessageV0) -> Bytes!String do
  ...
  let with_signatures = append_uint8(versioned, message.header.num_required_signatures)?
  let with_readonly_signers = append_uint8(with_signatures,
    message.header.num_readonly_signed_accounts)?
  let with_readonly_unsigned = append_uint8(with_readonly_signers,
    message.header.num_readonly_unsigned_accounts)?
  let with_static_count = append_short_u16(with_readonly_unsigned, static_count)?
  let with_static_keys = append_pubkeys(with_static_count, message.static_account_keys, 0)?
  let with_blockhash = append_bytes(with_static_keys, message.recent_blockhash.bytes)?
  let with_instruction_count = append_short_u16(with_blockhash, List.length(message.instructions))?
  let with_instructions = append_compiled_instructions(with_instruction_count,
    message.instructions,
    0,
    static_count,
    account_count)?
  let with_lookup_count = append_short_u16(with_instructions,
    List.length(message.address_table_lookups))?
  let bytes = append_address_table_lookups(with_lookup_count, message.address_table_lookups, 0)?
  if Bytes.length(bytes) > 1232 do
    Err("SOLANA_TX: serialized message exceeds 1232 bytes")
  else
    Ok(bytes)
...
fn append_zero_bytes(output :: Bytes, remaining :: Int) -> Bytes!String do
  if remaining <= 0 do
    ...
    append_zero_bytes(append_uint8(output, 0)?, remaining - 1)
  end
end
...
pub fn serialize_unsigned_legacy_transaction(message :: LegacyMessage) -> Bytes!String do
  serialize_unsigned_transaction(serialize_legacy_message(message)?,
    message.header.num_required_signatures)
end

pub fn serialize_unsigned_v0_transaction(message :: MessageV0) -> Bytes!String do
  serialize_unsigned_transaction(serialize_message_v0(message)?,
    message.header.num_required_signatures)
end
...
pub fn simulate_transaction_request(id :: Int,
  transaction :: Bytes,
  commitment :: String,
  replace_recent_blockhash :: Bool,
  min_context_slot :: Option<U64>) -> RpcRequest!String do
...
pub fn jupiter_instructions(value :: JupiterInstructionSet) -> List<Instruction> do
  List.new()
...
fn pubkey_strings(keys :: List<Pubkey>, index :: Int, values :: List<String>) -> List<String> do
  if index >= List.length(keys) do
    values
  else
    keys
      |> List.get(index)
      |> pubkey_string() |2> List.append(values) |3> pubkey_strings(keys, index + 1)
  end
end

fn compiled_program_ids(keys :: List<Pubkey>,
  instructions :: List<CompiledInstruction>,
  index :: Int,
  values :: List<String>) -> List<String> do
  if index >= List.length(instructions) do
    values
  else
    let instruction = instructions
    ...
    keys
      ...
      |> pubkey_string() |2> append_unique(values) |4> compiled_program_ids(keys,
      instructions,
      index + 1)
  end
end
...
fn lookup_table_keys(lookups :: List<AddressTableLookup>, index :: Int, keys :: List<String>) -> List<String> do
  if index >= List.length(lookups) do
    ...
    let lookup = lookups
      |> List.get(index)
    lookup.account_key
      |> pubkey_string() |2> List.append(keys) |3> lookup_table_keys(lookups, index + 1)
  end
end

fn loaded_address_strings(lookups :: List<AddressTableLookup>,
  index :: Int,
  writable :: Bool,
  ...
  if index >= List.length(lookups) do
    ...
    addresses
      |> pubkey_strings(0, keys) |4> loaded_address_strings(lookups, index + 1, writable)
  end
end

fn loaded_accounts(lookups :: List<AddressTableLookup>,
  index :: Int,
  writable :: Int,
  readonly :: Int) -> List<Int> do
  if index >= List.length(lookups) do
    [writable, readonly]
  else
    let lookup = lookups
      |> List.get(index)
    loaded_accounts(lookups,
      index + 1,
...
pub fn legacy_message_report_json(message :: LegacyMessage) -> String!String do
  ...
  Ok(json {
    ...
    programIds: compiled_program_ids(message.account_keys, message.instructions, 0, List.new()),
...
pub fn message_v0_report_json(message :: MessageV0) -> String!String do
  ...
  let loaded = loaded_accounts(message.address_table_lookups, 0, 0, 0)
  ...
  Ok(json {
    ...
    accountKeys: loaded_address_strings(message.address_table_lookups,
      0,
      false,
      loaded_address_strings(message.address_table_lookups, 0, true, static_keys)),
    programIds: compiled_program_ids(message.static_account_keys,
      message.instructions,
      0,
      List.new()),
    instructionCount: List.length(message.instructions),
    lookupTableKeys: lookup_table_keys(message.address_table_lookups, 0, List.new()),
...
fn collect_instructions(report :: JupiterInstructionReport,
  instructions :: List<Instruction>,
...
fn collect_optional_instruction(report :: JupiterInstructionReport,
  instruction :: Option<Instruction>) -> JupiterInstructionReport do
  case instruction do
    None -> report
    Some(value) -> report

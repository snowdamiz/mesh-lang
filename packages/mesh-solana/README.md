...
Version 0.2 adds legacy and v0 message serialization, address-table lookups,
recent-blockhash reads, compute-budget and SPL token instructions, zero-signature
transaction envelopes, bounded `simulateTransaction` requests and responses,
and reports that expose message programs/accounts without exposing transaction
bytes. The package validates Solana's 1,232-byte transaction limit and never
enables signature verification for unsigned simulation requests.

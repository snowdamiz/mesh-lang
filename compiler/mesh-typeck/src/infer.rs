...
fn infer_fn_def(
    ...
    // Compiler-generated test functions (`__test_body_*`, and `__test_describe_*`
    // for a describe's tests) are called by the test harness from the main thread,
    // which has an actor process entry (PID created by mesh_rt_init_actor). They may
    // contain `receive do ... end` blocks generated by the assert_receive
    // preprocessor. Inject ACTOR_MSG_TYPE_KEY so that typeck allows `receive`
    // expressions inside these generated functions.
    if fn_name.starts_with("__test_body_") || fn_name.starts_with("__test_describe_") {

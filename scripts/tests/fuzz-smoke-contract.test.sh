...
readonly ROOT_DIR
readonly SCRIPT_PATH="${ROOT_DIR}/scripts/fuzz-smoke.sh"
...
existing_dir="$(mktemp -d "${TMPDIR:-/tmp}/mesh-fuzz-contract.XXXXXX")"
[[ -d "${existing_dir}" && ! -L "${existing_dir}" ]] || fail "mktemp did not create a safe directory"
trap 'rmdir -- "${existing_dir}"' EXIT

status=0
output="$(bash "${SCRIPT_PATH}" "${existing_dir}" 1 2>&1)" || status=$?
...
printf 'fuzz smoke contract passed\n'

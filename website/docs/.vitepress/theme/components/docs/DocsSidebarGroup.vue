...
<template>
  ...
  <div v-if="item.collapsed === undefined" class="mb-5">
    <div class="text-xs font-semibold uppercase tracking-wider text-muted-foreground mb-2 px-2">
      {{ item.text }}
    </div>
  ...
  <Collapsible v-else v-model:open="isOpen" class="mb-5">
    <CollapsibleTrigger class="flex w-full items-center gap-1 px-2 py-1.5 text-xs font-semibold uppercase tracking-wider text-muted-foreground hover:text-foreground rounded-md transition-colors">
      <ChevronRight

...
must_haves:
  truths:
    ...
    - "Process struct has a terminate_callback field for cleanup logic before termination"
  artifacts:
    - path: "crates/snow-rt/src/actor/process.rs"
      provides: "Process Control Block (PCB) with state, stack, reduction counter, priority, terminate_callback"
...
5. **Process** struct (the PCB):
   ...
   - `terminate_callback: Option<TerminateCallback>` -- per user locked decision: actors can define cleanup logic that runs before full termination. This is an optional function pointer that the runtime invokes before exit propagation. The type is `pub type TerminateCallback = extern "C" fn(state_ptr: *const u8, reason_ptr: *const u8);` where state_ptr points to the actor's current state and reason_ptr points to a serialized ExitReason. Set to None by default; Plan 05 codegen will populate it when an actor defines a `terminate do ... end` block.
...
8. **TerminateCallback** type alias: `pub type TerminateCallback = extern "C" fn(state_ptr: *const u8, reason_ptr: *const u8);`
...
`cargo build -p snow-rt` compiles successfully. `cargo test -p snow-rt` passes (existing tests still green + any new unit tests for ProcessId generation uniqueness).
  ...
  <done>Process struct exists with all fields including terminate_callback. ProcessId generates unique IDs atomically. ProcessState and Priority enums defined. TerminateCallback type alias defined. Message placeholder exists. snow-rt builds with all new dependencies.</done>
</task>
...
<success_criteria>
1. Process Control Block defined with pid, state, priority, reductions, links, mailbox, terminate_callback
...
6. TerminateCallback type alias defined for actor cleanup before termination
7. All tests pass including multi-actor concurrency tests
</success_criteria>

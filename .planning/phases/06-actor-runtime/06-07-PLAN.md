...
must_haves:
  ...
  artifacts:
    - path: "crates/snowc/tests/e2e_actors.rs"
      provides: "End-to-end actor integration tests"
      contains: "actors_basic\\|actors_100k\\|actors_typed_pid"
...
<objective>
End-to-end integration testing and bug fixing for the complete actor runtime. Write Snow programs that exercise all actor features, compile them with snowc build, run the binaries, and verify correct output. Fix any integration issues discovered. Verify all 5 Phase 6 success criteria.
...
Output: E2E test suite for actors, 100K actor benchmark, any integration bug fixes, verification of all success criteria.
</objective>
...
All existing e2e tests still pass: `cargo test -p snowc --test e2e`.
  ...
  <done>6 E2E test programs created covering all 5 success criteria. Test harness compiles and runs all tests. 100K actor benchmark completes successfully. Typed Pid prevents wrong-type sends at compile time.</done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>Complete actor runtime with M:N scheduler, typed message passing, preemptive scheduling, process linking, and named registry -- integrated into the Snow compiler and tested end-to-end.</what-built>
  <how-to-verify>
...
All 5 criteria verified by automated E2E tests + human verification.
</verification>

<success_criteria>
1. All E2E actor tests pass (actors_basic, actors_messaging, actors_preemption, actors_linking, actors_typed_pid, actors_100k)
...
6. All existing Phase 1-5 tests still pass (zero regressions)
7. Human verifies complete actor system works
</success_criteria>

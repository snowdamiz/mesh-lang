...
must_haves:
  truths:
    ...
    - "Changeset.data(changeset) returns the original data Map passed to cast"
    - "Changeset.errors(changeset) returns the errors Map containing field-specific error messages"
    - "Changeset.changes(changeset) returns the changes Map containing only the allowed, filtered params"
  artifacts:
  ...
  key_links:
    ...
    - from: "crates/meshc/tests/e2e.rs"
      ...
      via: "e2e tests compile and run Mesh programs that invoke Changeset runtime functions through JIT"
      pattern: "changeset_cast_basic|changeset_validate_required|changeset_pipe_chain"
---
...
<task type="auto">
  ...
  <action>
    ...
    **Type Signature Mapping (MIR Level vs LLVM Level):**

    At the MIR/typeck level, functions use semantic types (Int, Atom, Bool). At the LLVM level (known_functions), ALL parameters are `Ptr` because everything is passed as `*mut u8` or `i64` (both 8 bytes, pointer-sized). This is NOT a discrepancy -- it is how the compiler pipeline works.

    Example mappings:
    | Function | MIR Level (infer.rs) | LLVM Level (known_functions in lower.rs) |
    |----------|---------------------|------------------------------------------|
    | `validate_length` | `(Ptr, Atom, Int, Int) -> Ptr` | `FnPtr([Ptr, Ptr, Ptr, Ptr], Ptr)` |
    | `validate_number` | `(Ptr, Atom, Int, Int, Int, Int) -> Ptr` | `FnPtr([Ptr, Ptr, Ptr, Ptr, Ptr, Ptr], Ptr)` |
    | `valid` | `(Ptr) -> Bool` | `FnPtr([Ptr], Ptr)` |

    The parameter COUNT must match between levels. The types differ because LLVM extern C ABI passes Int as i64 (same size as Ptr) and Bool as i64 (1/0). Verify by checking how existing `Query.limit(Ptr, Int)` maps to `FnPtr([Ptr, Ptr], Ptr)` in known_functions.

...
must_haves:
  ...
  artifacts:
    - path: "crates/snow-codegen/src/codegen/mod.rs"
      provides: "CodeGen struct with LLVM context, module, builder"
...
Output: Complete LLVM codegen for all MIR node types. An LLVM module containing all functions, type layouts, and runtime declarations. Object file emission capability.
</objective>
...
<task type="auto">
  <name>Task 1: LLVM type mapping, runtime intrinsic declarations, and CodeGen scaffolding</name>
  <files>
    crates/snow-codegen/src/codegen/mod.rs
    crates/snow-codegen/src/codegen/types.rs
    crates/snow-codegen/src/codegen/intrinsics.rs
...
Key methods:
- `new(context: &'ctx Context, mir: MirModule, opt_level: OptimizationLevel) -> Self` -- create module, builder, initialize native target, create target machine with default triple.
...
Store these as `FunctionValue` entries in the CodeGen's function map for easy lookup.
  ...
  <verify>
`cargo build -p snow-codegen` compiles. Write a test that creates a CodeGen with an empty MirModule, declares intrinsics, and verifies the LLVM module passes verification.
  </verify>
  <done>CodeGen struct with LLVM context/module/builder is operational. MirType maps to LLVM types correctly. Struct and sum type layouts are created. Runtime intrinsics are declared. Empty module passes LLVM verification.</done>
</task>
...
**Update lib.rs:**
Wire everything together in a `compile_to_object(parse: &Parse, typeck: &TypeckResult, output: &Path, opt_level: OptimizationLevel) -> Result<(), String>` function that:
1. Calls `lower_to_mir(parse, typeck)`
2. Compiles patterns
3. Creates CodeGen, runs compile, emits object file

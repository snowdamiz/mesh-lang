...
must_haves:
  ...
  key_links:
    ...
    - from: "crates/snow-codegen/src/mir/lower.rs"
      ...
      via: "Generated MIR calls snow_json_object_new/put/get, snow_json_as_*/snow_json_from_*, snow_json_from_list/from_map/to_list/to_map"
      pattern: "snow_json_object_new|snow_json_from_int|snow_json_as_int|snow_json_from_list|snow_json_to_list"
    - from: "crates/snow-codegen/src/mir/lower.rs"
      to: "crates/snow-codegen/src/mir/lower.rs"
...
<task type="auto">
  ...
  <action>
This task adds the core MIR generation for deriving(Json). All changes are in `crates/snow-codegen/src/mir/lower.rs`. The 4 collection runtime helpers (snow_json_from_list, snow_json_from_map, snow_json_to_list, snow_json_to_map) were already added in Plan 49-01 with full three-point registration -- this task only needs to emit MIR that calls them.
...
**E. Handle List<T> and Map<String, V> encoding using the collection helpers from Plan 49-01.**
...
Implement `emit_collection_to_json` -- this generates MIR that calls the runtime helpers registered in Plan 49-01:
```rust
            ...
            if let Ty::Con(con) = base.as_ref() {
                match con.name.as_str() {
                    "List" => {
                        ...
                        // Call snow_json_from_list(list_ptr, callback_fn_ptr)
                    ...
                    "Map" => {
                        ...
                        // Call snow_json_from_map(map_ptr, val_callback_fn_ptr)
                        let fn_ty = MirType::FnPtr(vec![MirType::Ptr, MirType::Ptr], Box::new(MirType::Ptr));
                        let callback_ty = MirType::FnPtr(vec![MirType::Ptr], Box::new(MirType::Ptr));
                        MirExpr::Call {

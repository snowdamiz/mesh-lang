...
1. Create website/.vitepress/config.ts:
   ...
   - Set appearance: true (enables built-in dark mode + FOUC prevention). NOTE: The `appearance: true` setting is critical for INFRA-05 (FOUC prevention). VitePress injects a blocking inline script in the HTML head that reads the theme preference from localStorage and applies the .dark class BEFORE the page renders. This prevents any flash of wrong theme on initial load. No manual script is needed -- it is fully handled by VitePress when appearance is truthy.
...
- No default VitePress styles appear (no sidebar, no default header)
  ...
  <verify>
Run `cd website && npm run dev` -- VitePress dev server starts without errors. Open the URL in a browser (default http://localhost:5173). The page shows "Mesh" heading with monochrome background. Check browser DevTools: body should have bg-background class resolved to oklch(1 0 0). Check that no vitepress/theme CSS is loaded (inspect stylesheets). Toggle dark mode manually via browser DevTools (add class "dark" to html element), then refresh the page -- confirm there is no flash of wrong theme (FOUC prevention working via VitePress appearance:true inline script).
  </verify>
  <done>
VitePress dev server runs. Custom Layout.vue renders without default theme styles. Tailwind utility classes resolve correctly (bg-background produces oklch(1 0 0) in light mode). shadcn-vue Button/DropdownMenu/Sheet components are installed in website/.vitepress/theme/components/ui/. The cn() utility exists and exports correctly. FOUC prevention works (no flash of wrong theme on page refresh).
  </done>
</task>
...
6. No FOUC on page load (appearance:true inline script present in HTML head)
</verification>
...
- No FOUC on initial page load (INFRA-05 satisfied by appearance:true)
</success_criteria>

...
<task type="auto">
  <name>Task 2: Add event and issue broadcasting to route handlers</name>
  <files>mesher/ingestion/routes.mpl</files>
  <action>
Modify `mesher/ingestion/routes.mpl` to broadcast notifications to streaming clients after successful event processing and issue state transitions.

**Event broadcasting (STREAM-01, STREAM-03, STREAM-04):**
...
Do NOT broadcast inside EventProcessor service -- broadcast AFTER the call returns in the route handler to avoid bottlenecking the single EventProcessor actor on network I/O.
  ...
  <verify>Build the Mesher application: `cd mesher && ../target/debug/meshc build .` (or run `cargo run -- build mesher/`). Verify no compilation errors. Check that routes.mpl uses Ws.broadcast in the correct locations (after EventProcessor returns, after issue state transitions). Verify the broadcast helpers follow single-expression case arm constraints.</verify>
  <done>Event processing broadcasts event notifications and issue count updates to project rooms. Issue state transitions (resolve, archive, unresolve, discard) broadcast issue update notifications. All broadcasting happens AFTER service calls return, not inside service actors. SDK ingestion flow is unchanged.</done>
</task>

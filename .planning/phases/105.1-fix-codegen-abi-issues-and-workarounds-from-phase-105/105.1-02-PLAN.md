...
Then add a value coercion in the general `codegen_expr` for Call nodes. When a call returns a pointer but the expected variable type is SumType or Struct, load the value through the pointer. This may already be handled -- check lines around 1505-1510 where the code checks:

```rust
let val = if matches!(ty, MirType::SumType(_) | MirType::Struct(_))
```

If this coercion doesn't handle pointer-to-struct loading, add it. The pattern:
```rust
if expected_ty is SumType or Struct, and actual LLVM value is PointerValue:
    let loaded = build_load(llvm_type_for_expected_ty, pointer_val, "loaded")
    use loaded
```

This coercion should be added in the variable binding / let-binding codegen path where a function call result is stored to a local variable.

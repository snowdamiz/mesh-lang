...
The `wrap_collection_to_string` function already handles Display for `List<Int>` when invoked via string interpolation (`"${my_list}"`). Direct `to_string(list)` function calls also dispatch through `wrap_collection_to_string` for Ptr-typed arguments (line ~3411-3420). This existing infrastructure should handle `List<String>`, `List<Bool>`, etc. because `resolve_to_string_callback` already handles `Ty::App(Con("List"), [inner_ty])` recursively.

For `debug(my_list)` dispatch: find the `"debug"` / `"inspect"` call handling in the lowerer (near where `"to_string"` calls are handled). Add a Ptr-type check: when the callee is `"debug"` or `"inspect"` and the argument has `MirType::Ptr` with a typeck type of `Ty::App(Con("List"), [elem_ty])`, emit a call to `wrap_collection_to_string` using the Debug callback (`resolve_to_string_callback` with debug=true, or the Debug variant of the element callback). For lists, Debug and Display use the same `[elem1, elem2, ...]` format, but Debug renders each element via its Debug impl (e.g., strings get quotes). If the existing `wrap_collection_to_string` already accepts a debug flag or uses the Debug callback when called from `debug()`, reuse that. Otherwise, add the dispatch by copying the `to_string` Ptr handling and changing the callback resolution to use Debug callbacks.

Verify both paths work with e2e tests.
...
Register all four tests in `crates/snowc/tests/e2e_stdlib.rs` following the existing test function pattern (compile + run + assert output).
  </action>
  <verify>
    `cargo test -p snow-codegen` passes (all existing tests + any new MIR tests).
    `cargo test -p snowc -- list_display_string` passes.
    `cargo test -p snowc -- list_debug` passes.
  ...
  <done>
    ...
    debug([1, 2, 3]) produces correct Debug output.
...
2. E2e tests compile and run: list_display_string, list_debug, list_eq, list_ord

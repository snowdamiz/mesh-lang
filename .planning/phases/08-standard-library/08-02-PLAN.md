...
must_haves:
  truths:
    ...
    - "Tuple.nth(tuple, index) returns the element at index, Tuple.first/second/size work correctly"
    - "Range.new(start, end) creates a range, Range.to_list converts to List, range can be piped to map/filter"
    - "Queue.new() creates an empty FIFO queue, Queue.push/pop/peek/size work correctly"
  ...
  artifacts:
    ...
    - path: "crates/snow-rt/src/collections/tuple.rs"
      provides: "Tuple utility functions"
      contains: "snow_tuple_nth"
    - path: "crates/snow-rt/src/collections/range.rs"
      provides: "Range type with iteration and list conversion"
      contains: "snow_range_new"
    - path: "crates/snow-rt/src/collections/queue.rs"
      provides: "FIFO Queue implementation"
      contains: "snow_queue_push"
...
<objective>
Implement the full collection suite -- List, Map, Set, Tuple utilities, Range, and Queue -- with all operations, including higher-order functions (map, filter, reduce) that accept Snow closures.

Purpose: Collections are the backbone of data processing in any language. This plan delivers the full collection suite with pipe-friendly APIs, enabling `list |> map(fn) |> filter(fn) |> reduce(init, fn)` chains. Tuple utilities provide ergonomic access to tuple elements, Range enables numeric iteration patterns, and Queue provides FIFO data structure for ordered processing. Higher-order function support (passing Snow closures to runtime functions) is the key technical challenge.

Output: Working List, Map, Set, Tuple utilities, Range, and Queue types with all operations, registered through the full compiler pipeline.
</objective>
...
**Tuple utilities (tuple.rs):**

Tuples already exist at the language level as product types. Tuple utilities provide runtime access functions for working with tuples as opaque values. Tuples are stored as GC-allocated structs with elements packed as u64 values (same uniform representation as List elements).

Implement these `extern "C"` functions:

- `snow_tuple_nth(tuple: *mut u8, index: i64) -> u64` -- return the element at the given index. The tuple is a pointer to a GC-allocated block where the first u64 is the arity (number of elements), followed by the element values. Panics if index is out of bounds.
- `snow_tuple_first(tuple: *mut u8) -> u64` -- shorthand for nth(tuple, 0)
- `snow_tuple_second(tuple: *mut u8) -> u64` -- shorthand for nth(tuple, 1)
- `snow_tuple_size(tuple: *mut u8) -> i64` -- return the arity (number of elements)

Note: Tuple.nth returns a dynamically typed value (u64). At the Snow level, the type checker handles the polymorphism. At runtime, all elements are 8-byte uniform.

**Range (range.rs):**

Define `SnowRange` as a GC-allocated struct: `{ i64 start, i64 end, i64 step }`. Step defaults to 1 for ascending ranges, -1 for descending.

Implement these `extern "C"` functions:

- `snow_range_new(start: i64, end: i64) -> *mut u8` -- create a range from start to end (inclusive). Step is +1 if start <= end, -1 if start > end.
- `snow_range_to_list(range: *mut u8) -> *mut u8` -- convert range to a SnowList of integers. Iterates from start to end (inclusive) using the step value.
- `snow_range_map(range: *mut u8, fn_ptr: *mut u8, env_ptr: *mut u8) -> *mut u8` -- apply closure to each element in range, return a SnowList of results. Same closure calling convention as List.map.
- `snow_range_filter(range: *mut u8, fn_ptr: *mut u8, env_ptr: *mut u8) -> *mut u8` -- filter range elements through closure, return SnowList of matching elements.
- `snow_range_length(range: *mut u8) -> i64` -- return number of elements in range without materializing.

This enables patterns like: `Range.new(1, 10) |> Range.to_list() |> map(fn(x) -> x * 2 end)` or directly `Range.new(1, 10) |> Range.map(fn(x) -> x * 2 end)`.

**Queue (queue.rs):**

Define `SnowQueue` as a GC-allocated FIFO queue. Internally use a `VecDeque<u64>` wrapped in an opaque box (same pattern as List). All operations return NEW queues (immutable).

Implement these `extern "C"` functions:

- `snow_queue_new() -> *mut u8` -- create an empty queue
- `snow_queue_push(queue: *mut u8, element: u64) -> *mut u8` -- return NEW queue with element added at the back
- `snow_queue_pop(queue: *mut u8) -> *mut u8` -- return a two-element result: a NEW queue with the front element removed, and the removed element. Represent as a GC-allocated pair `{ *mut u8 new_queue, u64 element }`. Panics if queue is empty. Alternative: return a tuple-like struct.
- `snow_queue_peek(queue: *mut u8) -> u64` -- return the front element without removing it. Panics if empty.
- `snow_queue_size(queue: *mut u8) -> i64` -- return number of elements
- `snow_queue_is_empty(queue: *mut u8) -> i8` -- 1 if empty, 0 otherwise

For `pop`, the simplest correct approach: return a pointer to a pair struct `{ queue_ptr: *mut u8, element: u64 }`. The Snow level can destructure this. Alternatively, provide two separate functions: `snow_queue_pop_value(queue) -> u64` (returns front element) and `snow_queue_pop_queue(queue) -> *mut u8` (returns queue without front).

Update `crates/snow-rt/src/collections/mod.rs` to include `pub mod tuple; pub mod range; pub mod queue;`.

Add comprehensive unit tests for all collection operations. Test List.map with a simple function pointer, test Map put/get cycles, test Set add/contains, test Tuple.nth with various arities, test Range.to_list for ascending/descending ranges, test Queue push/pop FIFO ordering.
  </action>
  <verify>
...
- "Tuple" -> { "nth" -> (Tuple, Int) -> A, "first" -> (Tuple) -> A, "second" -> (Tuple) -> A, "size" -> (Tuple) -> Int }
- "Range" -> { "new" -> (Int, Int) -> Range, "to_list" -> (Range) -> List<Int>, "map" -> forall B. (Range, (Int) -> B) -> List<B>, "filter" -> (Range, (Int) -> Bool) -> List<Int>, "length" -> (Range) -> Int }
- "Queue" -> { "new" -> () -> Queue<A>, "push" -> (Queue<A>, A) -> Queue<A>, "pop" -> (Queue<A>) -> (Queue<A>, A), "peek" -> (Queue<A>) -> A, "size" -> (Queue<A>) -> Int, "is_empty" -> (Queue<A>) -> Bool }
...
**MIR types -- types.rs**: In `resolve_con` and `resolve_app`, add cases for all collection types -> all resolve to `MirType::Ptr` (opaque pointer). Collections are just pointers at the LLVM level.

```rust
// In resolve_con:
"List" | "Map" | "Set" | "Range" | "Queue" | "Tuple" => MirType::Ptr,

// In resolve_app: if base_name is "List", "Map", "Set", "Queue", "Range", return MirType::Ptr
```
...
9. **Tuple.nth** -- Create a tuple, use Tuple.first and Tuple.second to extract elements, verify correct values
10. **Range.to_list** -- `Range.new(1, 5) |> Range.to_list()` produces list [1, 2, 3, 4, 5], verify length is 5
11. **Range.map** -- `Range.new(1, 3) |> Range.map(fn(x) -> x * 10 end)` produces [10, 20, 30]
12. **Queue FIFO** -- `Queue.new() |> Queue.push(1) |> Queue.push(2) |> Queue.peek()` returns 1 (first-in), verify Queue.size is 2
  </action>
  <verify>
...
6. Module-qualified names (List.map, Map.put, Set.add, Tuple.first, Range.new, Queue.push) work
7. `Tuple.first((1, 2, 3))` returns 1, `Tuple.size((1, 2, 3))` returns 3
8. `Range.new(1, 5) |> Range.to_list()` produces a list of [1, 2, 3, 4, 5]
9. Queue follows FIFO: push 1, push 2, peek returns 1, pop returns 1
</verification>
...
- Tuple utilities: Tuple.nth/first/second/size for accessing tuple elements
- Range: Range.new/to_list/map/filter/length for numeric iteration patterns
- Queue: Queue.new/push/pop/peek/size/is_empty for FIFO operations
...
- Range is pipeable: `Range.new(1, 10) |> Range.map(fn) |> filter(fn)`
